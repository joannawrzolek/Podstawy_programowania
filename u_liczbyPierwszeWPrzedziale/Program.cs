...
namespace u_liczbyPierwszeWPrzedziale
{
    class Program
    {
        ...
        {
            ulong n;
            Console.WriteLine("Podaj górną granicę ¬ przedziału, z którego chcesz wypisać ¬ wszystkie liczby pierwsze.");
            ...
            for (ulong i = 2; i <= m; i++)
            {
                if (!liczby[i - 2])
                {
                    index = i * i - 2; while (index < n - 1)
                    {
                       liczby[index] = true; index += i;
                    }
                }
            }

            Console.WriteLine("Liczby pierwsze z podanego ¬ przedziału to:");
            int j = 0; for (ulong i = 0; i < n - 1; i++)
            {
                if (!liczby[i])
                {
